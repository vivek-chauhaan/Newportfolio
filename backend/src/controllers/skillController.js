...
const createSkill = asyncHandler(async (req, res) => {
  const { name, proficiency, icon, displayOrder } = req.body;
...
const getSkillCategories = asyncHandler(async (_req, res) => {
  const categories = await SkillCategory.find().sort({
    displayOrder: 1,
    name: 1,
  });
...
const deleteSkillCategory = asyncHandler(async (req, res) => {
  ...
  if (inUse)
    throw new ApiError(
      409,
      "Cannot delete a category that still has skills assigned to it.",
    );
